...
本文件是 **Cursor / Claude Code / Codex CLI** 共用的开发宪法。工具入口只做指针，正文只维护这一份，禁止各工具各自写一套导致漂移。

- Cursor：`.cursor/skills/ai-coding/SKILL.md` → 读本文件
- Claude Code：`CLAUDE.md` → 读本文件
- Codex CLI：本文件即入口（Codex 默认加载仓库根 `AGENTS.md`）
...
**Step 0：环境自检（每次会话首次载入时执行）**。运行 `pnpm doctor`：
...
**Step 4：冒烟测试**。把功能真正跑起来验证关键路径：
- 服务端改动：至少命中一次对应 API（curl），看真实响应。
- CLI 改动：至少真实执行一次命令。
- **UI 交互改动：必须用 Playwright 实际测试交互**（真实点击/输入/断言界面反馈，spec 放 `apps/web/e2e/`），禁止只构建通过或只看截图。
- 禁止只编译通过就宣布完成。

**Step 5：验收**。运行 `pnpm typecheck && pnpm lint`，必须全绿；在回复中汇报冒烟测试的实际输出。低内存机器（可用内存 < 2GB）不要跑 `next build` / `pnpm rebuild`，用 `pnpm typecheck && pnpm lint` 验收即可，构建交给 GitHub Actions。
...
9. **git 门禁**：每次提交自动执行 `.githooks/pre-commit`（VERSION 同步 → lint-staged：prettier+eslint 只扫暂存文件 → `pnpm lint:deps` 禁循环依赖并强制 apps→packages → `pnpm typecheck`），任一失败则拒绝提交。禁止用 `--no-verify` 绕过；紧急情况必须使用时，在 commit message 中注明原因并尽快补绿。新机器克隆后 `pnpm install` 会自动挂上门禁（`prepare` 脚本设置 `core.hooksPath`）。类型导入必须用 `import type`（`@typescript-eslint/consistent-type-imports`）。
10. **模型署名**：commit message 必须带 trailer `AI-Model: <模型名>`（如 `AI-Model: kimi-k3` / `AI-Model: claude-sonnet` / `AI-Model: gpt-5`；人工提交写 `AI-Model: human`），由 `.githooks/commit-msg` 机械强制。设计文档同理，顶部作者行写明模型名。

## 技术栈速查

Next.js 15 App Router + React 19 + Tailwind v4（apps/web）；`@openai/agents` + `aisdk()` 桥接（packages/agent-runtime）；PGlite + pgvector（packages/data）；AI SDK providers（packages/model-gateway）。新增依赖前先查是否已有包提供该能力。

## 工具入口对照

| 工具 | 入口文件 | 怎么用 |
|---|---|---|
| Cursor | `.cursor/skills/ai-coding/SKILL.md` | 对话里 `/ai-coding` 或 Agent 自动发现 |
| Claude Code | `CLAUDE.md` | 启动时自动加载 |
| Codex CLI | `AGENTS.md`（本文件） | 启动时自动加载 |

规范变更只改本文件；三个入口文件禁止复制正文。
